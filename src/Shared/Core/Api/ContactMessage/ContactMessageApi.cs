...
namespace Nina.Api.ContactMessage
{
    public class ContactMessageApi : IContactMessageApi
    {
        ...
        public async Task<HttpResult<PagedList<ContactMessageDTO>>> GetMessagesAsync(string name = null, string email = null, int pageSize = 0, int pageIndex = 0)
        {
            var hr = await HttpClient.GetAsync(null, new Uri($"http://localhost:5002/api/contact/messages?name={name}&email={email}&pageSize={pageSize}&pageIndex={pageIndex}")).ConfigureAwait(false);
